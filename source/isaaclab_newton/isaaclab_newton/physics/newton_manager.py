...
class NewtonManager(PhysicsManager):
    ...
    """Cached index mapping for :meth:`update_visualization_state`. May legitimately be ``None``
    (identity mapping, no reordering needed) even after resolution -- see
    :attr:`_scene_data_mapping_resolved` for the "has this been computed yet" flag."""
    _scene_data_mapping_resolved: bool = False
    """Whether :attr:`_scene_data_mapping` has been computed at least once. Needed because
    :meth:`~isaaclab.scene_data.SceneDataProvider.create_mapping` itself returns ``None`` for the
    identity case, so ``_scene_data_mapping is None`` cannot distinguish "not yet computed" from
    ...
    def clear(cls):
        ...
        NewtonManager._scene_data_mapping_resolved = False
        NewtonManager._model_changes = set()
    ...
    def update_visualization_state(cls, scene_data_provider: SceneDataProvider | None = None) -> None:
        ...
        if not cls._scene_data_mapping_resolved:
            ...
            cls._scene_data_mapping_resolved = True

        cls._scene_data.transforms = cls._state_0.body_q
        # allow_passthrough=False forces get_transforms to copy element-wise into the array
        # referenced above (state_0.body_q's own buffer) rather than rebinding
        # cls._scene_data.transforms to reference the PhysX backend's own array. The default
        # zero-copy passthrough is only safe for callers that re-read cls._scene_data.transforms
        # after the call; consumers here (Newton visualizers/renderers) read state_0.body_q
        # directly, so the alias set up above must survive the call.
